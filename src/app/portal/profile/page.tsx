...
export default async function ProfilePage({
  ...
  const mailboxes = isStaff ? await getMyMailboxes() : [];
    ...
    <div className="space-y-6">
      ...
      {isStaff && (
        <section className="card max-w-4xl">
          <h2 className="font-bold text-navy">My Mailboxes</h2>

          {mailboxes.length > 1 && (
            <nav aria-label="Connected mailboxes" className="mt-3 border-b border-navy-100">
              <ul className="flex gap-1 overflow-x-auto">
                {mailboxes.map((m) => (
                  <li key={m.id}>
                    <Link
                      href={`/portal/profile?mailbox=${m.id}`}
          ...
          {active && (
            <div className="mt-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <ConfirmForm action={unlinkMailboxAction} confirmText={`Disconnect ${active.address} from your profile?`}>
                ...
                <ul className="mt-4 divide-y divide-gray-100">
                  ...
                  {(messages ?? []).map((m) => (
                    <li key={m.uid} className="py-2">
                      <Link
                        href={`/portal/profile/mailbox/${active.id}/${m.uid}`}
